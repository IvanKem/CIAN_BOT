...
def get_metro_distance(tr):
    ...
    metro_distance_walk = re.search(u'РїРµС€РєРѕРј', metro_distance)
    metro_distance_car = re.search(u'РЅР° РјР°С€РёРЅРµ', metro_distance)

    distance_type = ""
    if metro_distance_walk:
        distance_type = "РїРµС€РєРѕРј"
        on_car = False
    elif metro_distance_car:
        distance_type = "РЅР° РјР°С€РёРЅРµ"
        on_car = True
    else:
        print("error with distance to metro")
        print(metro_distance)
    distance_to_metro = str(metro_distance_minute) + " РјРёРЅ. " + distance_type
    if on_car:
...
def get_price_roubles(tr):
    ...
    if print_result: print("СЃС‚РѕРёРјРѕСЃС‚СЊ РІ СЂСѓР±Р»СЏС…:" + str(price_roubles))
...
def get_square_all(tr):
    ...
    mainSize = 0
    for square in td:
        main = re.search(u'РћР±С‰Р°СЏ:В \d+В Рј', square.get_text())
        if main != None:
            mainSize = re.search('\d+', main.group())
            mainSize = mainSize.group()
            if print_result: print("РћР±С‰Р°СЏ:" + str(mainSize))
...
def get_square_kitchen(tr):
    ...
    kitchenSize = 0
    for square in td:
        kitchen = re.search(u'РљСѓС…РЅСЏ:В \d+В Рј', square.get_text())
        if kitchen != None:
            kitchenSize = re.search('\d+', kitchen.group())
            kitchenSize = kitchenSize.group()
            if print_result: print("РљСѓС…РЅСЏ:" + str(kitchenSize))
...
def get_square_live(tr):
    ...
    liveSize = 0
    for square in td:
        live = re.search(u'Р–РёР»Р°СЏ:В \d+В Рј', square.get_text())
        if live != None:
            liveSize = re.search('\d+', live.group())
            liveSize = liveSize.group()
            if print_result: print("Р–РёР»Р°СЏ:" + str(liveSize))
    return int(liveSize)
...
def get_price_per_meter(tr):
    ...
    if print_result: print("С†РµРЅР° РІ СЂСѓР±Р»СЏС… Р·Р° РјРµС‚СЂ^2:" + str(pricePerMeter))
...
def get_building_type(tr):
    ...
    houseTypeStr = re.search(u'[-Р°-СЏРђ-РЇ]+', houseType)
...
def get_floor(tr):
    ...
    floor = int(floor)
    if print_result: print("Р­С‚Р°Р¶: " + str(floor))
    return floor
...
def get_floor_all(tr):
    ...
    if print_result and floorAll: print("Р­С‚Р°Р¶РЅРѕСЃС‚СЊ:" + str(floorAll))
    return floorAll
...
def get_additional_properties(tr):
    ...
    windowStreetExist = re.search('СѓР»РёС†Р°',window)
    if windowStreetExist != None:
        windowStreet = 1
    else : windowStreet = 0

    windowBackyardExist = re.search('РґРІРѕСЂ',window)
    if windowBackyardExist != None:
        windowBackyard = 1
    else: windowBackyard = 0

    phone = td[4].get_text().encode('utf-8')
    phoneExist = re.search('РґР°', phone)
    if phoneExist != None:
        phone = 1
    else: phone =0

    if print_result: print("Р•СЃС‚СЊ Р»Рё Р»РёС„С‚:" + str(lift))
    if print_result: print("Р•СЃС‚СЊ Р»Рё Р±Р°Р»РєРѕРЅ:" + str(balcon))
    if print_result: print("Р•СЃС‚СЊ Р»Рё РѕРєРЅРѕ РЅР° СѓР»РёС†Сѓ:" + str(windowStreet))
    if print_result: print("Р•СЃС‚СЊ Р»Рё РѕРєРЅРѕ РІРѕ РґРІРѕСЂ:" + str(windowBackyard))
    if print_result: print("Р•СЃС‚СЊ Р»Рё С‚РµР»РµС„РѕРЅ:" + str(phone))
